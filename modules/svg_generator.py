...
class SVGGenerator:
    ...
    def generate_svg(
        ...
        try:
            ...
            github_icon_svg = f"""
                <svg width="20" height="20" viewBox="0 0 16 16" fill="#{primary_text_color}">
            ...
            svg = f"""
                <svg width="{svg_width}" height="{svg_height}" xmlns="http://www.w3.org/2000/svg">
                    <rect width="{svg_width}" height="{svg_height}" fill="#{container_color}"/>
                    <style>
                        .commit-rect {{
                ...
                for day_index, day_commits in enumerate(week.get("days", [])):
                    ...
                    if "count" in day_commits:
                        if day_commits["count"] > 0:
                            ...
                            color = Color.darken(container_color, factor_color)
            ...
            svg += f"""
                   <g>
                        ...
                        <text id="username-text" class="commit-text" x="{27+20+5}" y="{text_y}" font-family="Arial" font-size="14" fill="#{primary_text_color}" font-weight="bold">
                            {username}
                        </text>
                        <text id="contributions-text" class="commit-text" x="{svg_width-25}" y="{text_y}" font-family="Arial" font-size="14" text-anchor="end" fill="#{secondary_text_color}">
